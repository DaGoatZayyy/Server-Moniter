...
def journal_events(limit=20):
    try: output = subprocess.check_output(["journalctl", "-n", str(limit), "--no-pager", "-o", "json"], text=True, stderr=subprocess.DEVNULL, timeout=5)
...
def snapshot(previous_net, previous_time, server_id):
    ...
    return {"server_id": server_id, "system": {"hostname": platform.node(), "platform": platform.system(), "release": platform.release(), "architecture": platform.machine(), "cpu_count": psutil.cpu_count(logical=True) or 0, "memory_total_mb": int(psutil.virtual_memory().total / 1024 / 1024)}, "metrics": {"cpu_percent": psutil.cpu_percent(interval=0.5), "memory_percent": psutil.virtual_memory().percent, "gpu_percent": gpu_percent(), "disk_percent": psutil.disk_usage("/").percent, "download_mbps": round(down, 2), "upload_mbps": round(up, 2), "uptime_seconds": int(time.time() - psutil.boot_time())}, "processes": process_snapshot(), "logs": logs, "errors": errors}, net, now
