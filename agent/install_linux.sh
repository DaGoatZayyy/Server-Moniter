...
if [[ "${EUID}" -ne 0 ]]; then
  echo "Run this installer as root (for example: sudo bash install_linux.sh)." >&2
  exit 1
fi

: "${SERVER_ID:?Set SERVER_ID to the server ID from the ServerMonitor dashboard}"
: "${TOKEN:?Set TOKEN to the one-time enrollment secret}"
: "${URL:?Set URL to the full /api/agent/heartbeat endpoint}"
INTERVAL="${INTERVAL:-5}"
...
"$INSTALL_DIR/venv/bin/python" -m pip install --disable-pip-version-check --upgrade psutil requests
...
EOF
chown root:"$SERVICE_USER" "$INSTALL_DIR/agent.json"
chmod 0640 "$INSTALL_DIR/agent.json"
